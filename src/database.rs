...
/// Applies all schema migrations and seeds default settings.
pub fn init_db(path: &str) -> Result<Connection> {
    ...
    // Try to checkpoint WAL so our reader can see committed data
...
pub fn seed_defaults(conn: &Connection) -> Result<()> {
    ...
    let _ = conn.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('app.privacy_protection_kinds', 'phone,idcard,email,secret')",
        [],
    );
    let _ = conn.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('app.privacy_protection_custom_rules', '')",
        [],
    );
    let _ = conn.execute(
    ...
    let _ = conn.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('app.win_clipboard_disabled', 'false')",
        [],
    );
    let _ = conn.execute(
